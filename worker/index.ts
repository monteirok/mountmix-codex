  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/_vinext/image") {
      ...
      const imageHandlers: ImageHandlers = {
        fetchAsset: (path) => assetFetcher.fetch(new Request(new URL(path, request.url))),
      };

      const imageTransformer = env.IMAGES;
      if (imageTransformer) {
        imageHandlers.transformImage = async (body, { width, format, quality }) => {
          const result = await imageTransformer.input(body).transform(width > 0 ? { width } : {}).output({ format, quality });
